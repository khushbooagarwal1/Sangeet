    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <div class="row">
                 <!--  page header -->
                <div class="col-lg-12">
                    <h1 class="page-header">My Playlist</h1>
                </div>
                 <!-- end  page header -->
            </div>
            	<div class="row">
			...
			while($output = mysql_fetch_assoc($res))
			{
				if($output['share'] == 'Y')
				{
					$share="Hide From Public";
				}
				else
				{
					$share="Make it Public";
				}
				$query1 = "SELECT * FROM user_detail WHERE user_id =".$output['owner'].";";
				...
				echo '<td><audio controls><source type="audio/mpeg"src="'.$file_loc.'"/></audio></td>';
				echo '<td>'.$output1['first_name'].' '.$output1['last_name'].'</td>';
				if($output['owner'] == $_SESSION['user_id'])
				{
					echo '<td><div class="btn-group"><button type="button" class="btn btn-default btn-xs dropdown-toggle" data-toggle="dropdown"> Actions <span class="caret"></span> </button><ul class="dropdown-menu pull-left" role="menu"><li><a href="process_playlist.php?&opt=share&public='.$output['share'].'&name='.$output['file_name'].'">'.$share.'</a></li><li><a href="process_playlist.php?&opt=delete&name='.$output['file_name'].'">Delete From Playlist</a></li></li></li><li class="divider"></li><li><a href="process_playlist.php?&opt=download&name='.$output['file_name'].'">Download</a></li></ul></div></td></tr>';
				}
				else
				{
					echo '<td><div class="btn-group"><button type="button" class="btn btn-default btn-xs dropdown-toggle" data-toggle="dropdown"> Actions <span class="caret"></span> </button><ul class="dropdown-menu pull-left" role="menu"><li><a href="process_playlist.php?&opt=delete&name='.$output['file_name'].'">Delete From Playlist</a></li></li><li class="divider"></li><li><a href="process_playlist.php?&opt=download&name='.$output['file_name'].'">Download</a></li></ul></div></td></tr>';
				}
				$count = $count+1;
